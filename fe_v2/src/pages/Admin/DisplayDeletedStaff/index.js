...
const DisplayDeletedStaff = () => {
    ...
    if (listStaff.length > 0) {
      return (
        data.map((value, index) => {
          if (index == 0)
            return <th><input id="checkbox-all" type="checkbox" onChange={e => isChecked(true, e)}></input></th>
          else
            return <th>{value}</th>
        })
      )
  ...
  const restoreStaff = (idStaff, nameStaff) => {
    if (window.confirm(`Bạn có chắc chắn muốn khôi phục ${nameStaff.toUpperCase()} không?`)) {
  ...
  const handleDestroyStaff = ((idStaff, nameStaff) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa VĨNH VIỄN ${nameStaff.toUpperCase()} không?`)) {
  ...
  const isChecked = ((isAll, e) => {
    ...
    const childIsChecked = document.querySelectorAll("input.checkbox-item:checked")
    if (isAll) {
  ...
  const getStaffValues = () => {
    return listStaff.map((staff, indexStaff) => {
      return (
        <tr>
          <td>
            <input className="checkbox-item" type="checkbox" value={staff._id} onChange={e => isChecked(false, e)}></input>
          </td>
          <td>{indexStaff + 1}</td>
          <td>{staff.name}</td>
          <td>{staff.email}</td>
          <td>{staff.password || "Tài khoản Google"}</td>
          <td id="edit-staff" onClick={e => restoreStaff(staff._id, staff.name)}>
            <i class="fa-solid fa-trash-can-arrow-up"></i>
          </td>
          <td id="destroy-staff" onClick={e => handleDestroyStaff(staff._id, staff.name)}>
  ...
  const restoreAllStaff = () => {
    const childIsChecked = document.querySelectorAll("input.checkbox-item:checked")
    if (window.confirm(`Bạn có chắc chắn muốn khôi phục ${childIsChecked.length} nhân viên không?`)) {
      childIsChecked.forEach(item => {
        Axios.put(`${process.env.REACT_APP_API}/admin/restoreStaff/${item.value}`)
          .then((res) => {
            setIsRestore(!isRestore)
          })
          .catch((err) => {
  ...
  const handleDestroyAllStaff = () => {
    const childIsChecked = document.querySelectorAll("input.checkbox-item:checked")
    if (window.confirm(`Bạn có chắc chắn muốn xóa vĩnh viễn ${childIsChecked.length} nhân viên không?`)) {
      childIsChecked.forEach(item => {
        Axios.delete(`${process.env.REACT_APP_API}/admin/destroyStaff/${item.value}`)
          .then((res) => {
            setIsRestore(!isRestore)
          })
          .catch((err) => {
  ...
  return (
    <div className="container-staff">
      ...
      <div className="btn-add">
        {isDisplayButton && <>
          <Button text="Khôi phục tất cả" type={0} onClick={restoreAllStaff} />
          <Button text="Xóa tất cả" type={0} onClick={handleDestroyAllStaff} />
        </>}
